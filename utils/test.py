...
def test_retrieved_chunks(rag_engine, test_query, collection, top_k=3, min_score=0.3):
    """
    Test the retrieval of chunks for a given query.
    
    Args:
        rag_engine: The RAG engine instance.
        test_query (str): The test query string.
        collection (str): The collection name to search within.
        top_k (int): Number of chunks to retrieve.
        min_score (float): Minimum similarity score for filtering chunks.
    ...
    for i, chunk_info in enumerate(retrieved_chunks):
        print(f"\n--- Chunk {i+1} ---")
        source = chunk_info['metadata'].get('filename', chunk_info['metadata'].get('url', 'Unknown'))
        chunk_text = chunk_info.get('text') or chunk_info.get('page_content', 'No text available.')

        print(f"Source: {source}")
        print(f"Similarity Score: {chunk_info['score']:.4f}")
        print(f"Content: {chunk_text[:300]}...")  # Show first 300 characters for preview

        # Extract keywords if needed
        keywords = extract_keywords(chunk_text)
        keywords = clean_keywords(keywords)
        print(f"Keywords: {', '.join(keywords)}" if keywords else "No keywords found.")
...
if __name__ == "__main__":
    # Ensure `rag_engine` and `st.session_state.current_collection` exist
    if "rag_engine" in st.session_state and st.session_state.current_collection:
        ...
        print("⚠️ RAG Engine or Collection is not set.")
